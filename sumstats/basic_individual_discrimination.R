...
metafiles = list.files(path,pattern=suffix_pattern,full.names = T,recursive=T) ## .Table.1
metafiles=metafiles[1:length(metafiles)]
metafiles=sub("//","/",metafiles)
#shortnames = sub(".selections.txt","",basename(metafiles))
shortnames = sub(suffix_pattern,"",basename(metafiles))
...
if(runSumstat==T){
  ...
  print("convert ffreq cols to one col")
  ...
  newcols_raw=lapply(big_table$`Begin File`,FUN=function(x){
    ...
    if(length(y)==4) {
      ...
      print(x)
      stop("Error! This file does not have correct file naming!")
    }
    return(z)
  })
