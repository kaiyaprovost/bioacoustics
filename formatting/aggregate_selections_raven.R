...
slices_file = "/Users/kprovost/Documents/Postdoc_Working/MMRR/WAVS/Wave/predicted_slices_MMRR_14Jun2023.txt"
...
#pred_list = pred_list[(grepl("Table",pred_list))] ## optional
...
for (predicted_selections in sample(sort(pred_list[1:length(pred_list)]))) {
  ...
  try({
    ...
    pred_data_type_write = c("PRED", basename(predicted_selections), pred_data_type)
    
    outfile = gsub(suffix, newsuffix, predicted_selections)
    
    testrow = as.numeric(pred_data_write[3:length(pred_data_write)])
    testrowtype = (pred_data_type_write[3:length(pred_data_type_write)])
    ...
    ## with type
    runlengthtype = rle(testrowtype)
    stopstype = cumsum(runlengthtype$lengths) ## identical to the other one?
    startstype = stopstype+1
    startstype = c(1,startstype)
    startstype = startstype[1:length(stopstype)]
    are_sylltype = which(runlengthtype$values!=0)
    sylltype = runlengthtype$values[are_sylltype]
    onsetstype = startstype[are_sylltype]
    offsetstype = stopstype[are_sylltype]
    ...
    onsetstype_s = onsetstype * timebin_dur
    offsetstype_s = offsetstype * timebin_dur
    too_short_type = which((offsetstype_s - onsetstype_s) < min_segment_dur)
    
    if (length(too_short) >= 1) {
      onsets_s = onsets_s[-too_short]
      offsets_s = offsets_s[-too_short]
      onsetstype_s = onsetstype_s[-too_short]
      offsetstype_s = offsetstype_s[-too_short]
      pred_df=pred_df[-too_short,]
      sylltype=sylltype[-too_short]
    }
