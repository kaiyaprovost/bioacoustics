...
if(do_sumstats==T){
  ...
  filenames = list.files(path = getwd(), 
                         pattern = pattern,full.names = T,recursive=T)
  filenames = filenames[!(grepl("wav_",filenames))]
  
  generate_sumstats = function(filenames,suffix,import_raven=T,
  ...
  generate_sumstats(filenames,suffix = pattern,verbose=T)
  ...
  combine_finished_substats = function(finished_files,date=format(Sys.time(), "%d%b%Y"),workingdirectory="~/",verbose=F){
    ...
    if(file.exists(pcafile)) {
      ...
      max_pcs = min(10,ncol(pca$x))
      pca_data = cbind(finished[,which((colnames(finished) %in% metadata_cols))],finished_pca,pca$x[,1:max_pcs])
  ...
  print("combine_finished_substats")
...
if(do_soundshape==T){
  ...
  path=mainpath  #c("/Users/kprovost/Documents/OneDrive - The Ohio State University/Song/SoundShape/complexity_metric/")
  ...
  run_soundshape(path=path,pattern=pattern,doplot=F,verbose=T,listfiles=listfiles,outpath=outpath,
                overwrite=F,checkpointNumber=1,redo_eig=F,alignCheckpoint=1,eigcheckpoint=1,shuffle=F,rev=F,skipCutwave=F,skipAlign=F,tpsname="eig.sample",
                manualcombinetps=F)
